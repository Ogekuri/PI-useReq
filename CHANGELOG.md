...
- pin GitHub Actions runner to ubuntu-24.04 [useReq] *(release-npm)*
  - Pin both jobs (check-branch, build-release) to the ubuntu-24.04 label so the release pipeline compiles on Ubuntu 24.04 LTS tooling, immune to the GitHub ubuntu-latest re-target to Ubuntu 26.04 (Oct-Nov 2026 breaking change). Add a reproducer unit test in tests/release-workflow.test.ts (TST-039) asserting every runs-on declaration is ubuntu-24.04 and that ubuntu-latest is absent. Update pi-usereq/docs/WORKFLOW.md release execution units to document the pinned Ubuntu 24.04 hosted runner.
- Change version.

### ◀️  Revert
- Roll back branch to 868345f5 (868345f5c7cf1681ae77c3b0286c015c9986a67e).
