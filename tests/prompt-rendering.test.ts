...
test("pi.dev-aware prompts inject manifest conformance rules and API mandates when the manifest exists", () => {
  ensureBundledResourcesAccessible();
  const projectBase = process.cwd();
  const config = getDefaultConfig(projectBase);
  ...
  assert.match(rendered, /authoritative API contract/);
  assert.match(rendered, /new or modified pi\.dev CLI integrations MUST comply with the APIs it describes\./);
...
test("pi.dev-aware prompts stay unchanged when the manifest is absent", () => {
  ...
  try {
    ...
    assert.doesNotMatch(rendered, /authoritative API contract/);
