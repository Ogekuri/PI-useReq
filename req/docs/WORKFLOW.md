        ...
        - `handleExtensionStatusEvent(...)`: process the replayed `session_start` payload through shared status telemetry helpers [`src/index.ts`]
          ...
          - `applyConfiguredPiUsereqTools(...)`: enforce configured active tools against the offline runtime inventory [`src/index.ts`]
            ...
            - `RecordingExtensionAPI.setActiveTools(...)`: replace active-tool set with runtime-visible names [`scripts/lib/recording-extension-api.ts`]
              - `RecordingExtensionAPI.getAllTools(...)`: expose supported builtin plus extension tool inventory [`scripts/lib/recording-extension-api.ts`]
          - `setPiUsereqStatusConfig(...)`: cache docs/tests/src values for later status renders [`src/core/extension-status.ts`]
          - `updateExtensionStatus(...)`: refresh stored context usage, timers, and rendered status text [`src/core/extension-status.ts`]
            ...
            - `renderPiUsereqStatus(...)`: render the docs/tests/src/tools/context/elapsed/last footer with threshold-specific context overlays [`src/core/extension-status.ts`]
      ...
      - `withProcessCwd(...)`: execute the named command under the requested cwd [`scripts/lib/extension-debug-harness.ts`]
        ...
        - `configurePiUsereq(...)`: execute the interactive configuration menu [`src/index.ts`]
          ...
          - `getConfiguredEnabledPiUsereqTools(...)`: canonicalize configured enabled tools for status rendering [`src/index.ts`]
            - `normalizeEnabledPiUsereqTools(...)`: canonicalize configurable active tools [`src/core/pi-usereq-tools.ts`]
          - `formatPiUsereqStatus(...)`: render the colored single-line docs, tests, source, and tool-count status payload [`src/index.ts`]
...
- Lifecycle/trigger:
  ...
  - Threads: no explicit threads detected.
- Internal Call-Trace Tree:
  - `piUsereqExtension(...)`: initialize resources, register commands and tools, and attach lifecycle-hook telemetry wrappers [`src/index.ts`]
  ...
  - `configurePiUsereq(...)`: interactive configuration menu runtime root [`src/index.ts`]
    ...
    - `saveProjectConfig(...)`: persist config on save-and-close [`src/index.ts`]
      ...
      - `saveConfig(...)`: persist config JSON [`src/core/config.ts`]
        - `getProjectConfigPath(...)`: resolve config file path [`src/core/config.ts`]
    - `setPiUsereqStatusConfig(...)`: cache the just-saved docs/tests/src values for status rendering [`src/core/extension-status.ts`]
    - `renderPiUsereqStatus(...)`: render the docs/tests/src/tools/context/elapsed/last footer after save-and-close with threshold-specific context overlays [`src/core/extension-status.ts`]
      - `createStatusThemeAdapter(...)`: normalize label, value, and background renderers for status output [`src/core/extension-status.ts`]
      - `buildPiUsereqStatusText(...)`: compose the single-line footer payload [`src/core/extension-status.ts`]
        - `formatStatusField(...)`: render one colored text field [`src/core/extension-status.ts`]
        - `formatRenderedStatusField(...)`: render one pre-styled field [`src/core/extension-status.ts`]
    ...
    - `applyConfiguredPiUsereqTools(...)`: re-apply active tools after reset [`src/index.ts`]
      ...
      - `getPiUsereqStartupTools(...)`: enumerate configurable tools from runtime inventory [`src/index.ts`]
- External Boundaries:
  - pi ExtensionAPI plus command-context APIs for command registration, tool registration, user-message delivery, session events, active-tool mutation, editor updates, notifications, selections, and text input.
