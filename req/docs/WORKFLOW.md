    ...
    - `replayCommand(...)`: replay a recorded command handler and capture prompt/UI side effects [`scripts/lib/extension-debug-harness.ts`]
      ...
      - `withProcessCwd(...)`: execute the named command under the requested cwd [`scripts/lib/extension-debug-harness.ts`]
        ...
        - `deliverPromptCommand(...)`: dispatch prompt replay according to `reset-context` [`src/index.ts`]
          ...
          - `RecordingCommandContext.newSession(...)`: emulate `/new`-equivalent prompt-session replacement and run setup message injection [`scripts/lib/recording-extension-api.ts`]
            - `RecordingExtensionAPI.recordSessionUserMessage(...)`: capture the setup-appended reset-session prompt as a recorded user message [`scripts/lib/recording-extension-api.ts`]
  ...
  - `piUsereqExtension(...)`: initialize resources, register commands and tools, attach session-start behavior [`src/index.ts`]
    ...
    - `registerPromptCommands(...)`: register one command per prompt name [`src/index.ts`]
      ...
      - `deliverPromptCommand(...)`: dispatch the rendered prompt according to `reset-context` [`src/index.ts`]
        - External boundaries: `ctx.waitForIdle(...)`, `ctx.newSession(...)`, `sessionManager.appendMessage(...)`, and `pi.sendUserMessage(...)`; reset-enabled delivery appends the prompt during new-session setup before session handoff, while reset-disabled delivery sends the prompt into the current session.
