  ...
  - `main(...)`: parse argv, dispatch inspect/session-start/command/tool/sdk-smoke, and serialize the selected report [`scripts/debug-extension.ts`]
    ...
    - `replayTool(...)`: replay a recorded tool execute handler and capture content/details [`scripts/lib/extension-debug-harness.ts`]
      ...
      - `withProcessCwd(...)`: execute the named tool under the requested cwd [`scripts/lib/extension-debug-harness.ts`]
        ...
        - `runFilesReferences(...)`: explicit-file tool failure path [`src/core/tool-runner.ts`]
        - `runFilesCompress(...)`: explicit-file compression failure path when no file can be compressed [`src/core/tool-runner.ts`]
        - `collectSourceFiles(...)`: gather configured source files for project references and construct-search scopes [`src/core/tool-runner.ts`]
        - `runReferences(...)`: project-scan tool failure path [`src/core/tool-runner.ts`]
        - `runCompress(...)`: project-scan compression failure path when no file can be compressed [`src/core/tool-runner.ts`]
...
- Internal Call-Trace Tree:
  - `piUsereqExtension(...)`: initialize resources, register commands and tools, attach session-start behavior [`src/index.ts`]
    ...
    - `registerAgentTools(...)`: register structured pi tools whose execute callbacks reuse internal runner families and agent-oriented payload builders [`src/index.ts`]
      ...
      - `runFilesCompress(...)`: explicit-file compression failure path when no file can be compressed [`src/core/tool-runner.ts`]
      - `collectSourceFiles(...)`: gather configured source files for project references and construct-search scopes [`src/core/tool-runner.ts`]
      - `runReferences(...)`: project-scoped tool failure path [`src/core/tool-runner.ts`]
      - `runCompress(...)`: project-scoped compression failure path when no file can be compressed [`src/core/tool-runner.ts`]
      - `runTokens(...)`: validate zero-processable canonical-doc token requests on failure paths [`src/core/tool-runner.ts`]
