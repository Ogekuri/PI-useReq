        ...
        - `handleExtensionStatusEvent(...)`: process the replayed `session_start` payload through shared status telemetry helpers [`src/index.ts`]
            ...
            - `renderPiUsereqStatus(...)`: render the docs/tests/src/tools/context/elapsed/last/beep/sound footer with threshold-specific context overlays [`src/core/extension-status.ts`]
              ...
              - `buildPiUsereqStatusText(...)`: compose the single-line footer payload [`src/core/extension-status.ts`]
                ...
                - `formatContextUsageBar(...)`: render the 5-cell context bar or threshold-specific overlay [`src/core/extension-status.ts`]
                  - `resolveContextUsageOverlay(...)`: choose `claer` or `full!` overlay states from context usage thresholds [`src/core/extension-status.ts`]
          ...
          - `runPiNotifyEffects(...)`: dispatch configured terminal beeps and successful-run sound hooks on `agent_end` [`src/core/pi-notify.ts`]
            ...
            - `runPiNotifySoundCommand(...)`: execute the configured successful-run sound command when sound is enabled [`src/core/pi-notify.ts`]
              ...
              - `substitutePiNotifyInstallPath(...)`: replace `$install-path` before shell execution [`src/core/pi-notify.ts`]
    ...
    - `replayCommand(...)`: replay a recorded command handler and capture prompt/UI side effects [`scripts/lib/extension-debug-harness.ts`]
      ...
      - `withProcessCwd(...)`: execute the named command under the requested cwd [`scripts/lib/extension-debug-harness.ts`]
        ...
        - `configurePiUsereq(...)`: execute the interactive configuration menu [`src/index.ts`]
          - `buildPiUsereqConfigOverview(...)`: summarize docs, tests, src, tools, beep, and sound values for the main menu [`src/index.ts`]
          ...
          - `configurePiNotifyMenu(...)`: interactive pi-notify editor [`src/index.ts`]
            - `renderPiNotifyReference(...)`: render notification configuration reference text [`src/index.ts`]
...
- Internal Call-Trace Tree:
  ...
  - `configurePiUsereq(...)`: interactive configuration menu runtime root [`src/index.ts`]
    ...
    - `renderPiUsereqStatus(...)`: render the docs/tests/src/tools/context/elapsed/last/beep/sound footer after save-and-close with threshold-specific context overlays [`src/core/extension-status.ts`]
      - `createStatusThemeAdapter(...)`: normalize label, value, and background renderers for status output [`src/core/extension-status.ts`]
      - `buildPiUsereqStatusText(...)`: compose the single-line footer payload [`src/core/extension-status.ts`]
        - `formatStatusField(...)`: render one colored text field [`src/core/extension-status.ts`]
        - `formatRenderedStatusField(...)`: render one pre-styled field [`src/core/extension-status.ts`]
