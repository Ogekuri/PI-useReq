  ...
  - `main(...)`: parse argv, dispatch inspect/session-start/command/tool/sdk-smoke, and serialize the selected report [`scripts/debug-extension.ts`]
    ...
    - `replayCommand(...)`: replay a recorded command handler and capture prompt/UI side effects [`scripts/lib/extension-debug-harness.ts`]
      ...
      - `withProcessCwd(...)`: execute the named command under the requested cwd [`scripts/lib/extension-debug-harness.ts`]
        ...
        - `prepareReqResetCommandExecution(...)`: derive base-path recovery, normalize to the main repository worktree, and resolve generated cleanup facts for `req-reset` [`src/core/req-reset-command.ts`]
          - `resolveReqResetMainWorktreeRoot(...)`: normalize the cleanup git root to the main repository worktree [`src/core/req-reset-command.ts`]
        ...
        - `executeReqResetCommandExecution(...)`: preserve transcript, restore `base-path`, ensure the main worktree stays on the main branch, return the pi CLI execution surfaces to the main base path before any matching worktree directory is removed, and force-remove matching worktrees plus branches for `req-reset` [`src/core/req-reset-command.ts`]
          ...
          - `ensureReqResetMainBranch(...)`: switch the main worktree back to the main branch when HEAD rests on a generated branch [`src/core/req-reset-command.ts`]
            - `resolveReqResetMainBranchName(...)`: resolve the repository main branch from origin HEAD plus local conventions [`src/core/req-reset-command.ts`]
          ...
          - `redirectReqResetExecutionToMainBranch(...)`: switch the pi CLI session, process cwd, and context cwd back to the main base path before any matching worktree directory is removed [`src/core/req-reset-command.ts`]
            - `reqResetExecutionPathNeedsRedirect(...)`: test whether one live execution path must be re-anchored to the main base path [`src/core/req-reset-command.ts`]
            - `resolveReqResetParentSessionFile(...)`: resolve the original base-path session file from the active session header `parentSession` link [`src/core/req-reset-command.ts`]
              - `readReqResetSessionHeader(...)`: parse the persisted header record from one session file [`src/core/req-reset-command.ts`]
            - `resolveReqResetMainSessionFile(...)`: resolve the most recent persisted session file rooted at the main base path [`src/core/req-reset-command.ts`]
            - `switchPromptCommandSession(...)`: switch the active session to the base-path session file with stale-context tolerance [`src/core/prompt-command-runtime.ts`]
          ...
          - `restoreReqResetExecutionCwd(...)`: restore deleted process and context cwd surfaces after worktree removal [`src/core/req-reset-command.ts`]
        ...
        - `executeReqReferencesCommandExecution(...)`: regenerate `REFERENCES.md`, stage only the target file, create the fixed commit through the guarded commit helper when staged changes exist, and verify clean repository state for `req-references` [`src/core/req-references-command.ts`]
          ...
          - `runGuardedGitCommit(...)`: skip commit creation or run `git commit -m` based on a staged-changes precheck [`src/core/req-references-command.ts`]
            - `hasStagedChangesForPaths(...)`: inspect the cached index via `git diff --cached --quiet` for staged differences on the target paths [`src/core/req-references-command.ts`]
            - `runCapture(...)`: run git add, diff inspection, guarded commit, and status subprocesses [`src/core/req-references-command.ts`]
...
- External Boundaries:
  - Dynamic module loading for the target extension entry and optional `@earendil-works/pi-coding-agent` SDK package.
...
- Internal Call-Trace Tree:
  - `piUsereqExtension(...)`: initialize resources, register commands, tools, shortcuts, and lifecycle-hook telemetry wrappers [`src/index.ts`]
    ...
    - `registerReqResetCommand(...)`: register the specialized non-agentic `req-reset` command that accepts any workflow state, restores `base-path`, force-removes matching generated worktrees plus branches, clears recoverable prompt state when restoration succeeds, and returns to `idle` without worktree creation or LLM initialization [`src/index.ts`]
        ...
        - `renderPiUsereqStatus(...)`: refresh live context usage and render the extension/status/branch/context/elapsed/sound footer with CLI-theme token overlays [`src/core/extension-status.ts`]
          ...
          - `buildPiUsereqStatusText(...)`: compose the single-line footer payload with runtime extension identity, workflow state, branch, context gauge, elapsed semantics, and the active runtime sound status field [`src/core/extension-status.ts`]
            ...
            - `formatElapsedStatusValue(...)`: compose `⏱︎`, `⚑`, and `⌛︎` timer segments [`src/core/extension-status.ts`]
              - `formatCompletedStatusDuration(...)`: render completed timers or the unset placeholder [`src/core/extension-status.ts`]
              - `formatStatusDuration(...)`: render `M:SS` durations [`src/core/extension-status.ts`]
      - `prepareReqResetCommandExecution(...)`: derive the generated cleanup matcher, normalize to the main repository worktree, and resolve the optional recovery plan for `req-reset` [`src/core/req-reset-command.ts`]
        - `resolveReqResetMainWorktreeRoot(...)`: normalize the cleanup git root to the main repository worktree [`src/core/req-reset-command.ts`]
        - `buildReqResetWorktreeNamePattern(...)`: derive the generated worktree-name matcher from the configured prefix plus repository basename [`src/core/req-reset-command.ts`]
      - `executeReqResetCommandExecution(...)`: preserve transcript, restore `base-path`, ensure the main worktree stays on the main branch, return the pi CLI execution surfaces to the main base path before any matching worktree directory is removed, and force-remove matching worktrees plus branches [`src/core/req-reset-command.ts`]
        - `preservePromptCommandExecutionTranscript(...)`: copy execution-session records into the original session file when recovery plan data exists [`src/core/prompt-command-runtime.ts`]
        - `restorePromptCommandExecution(...)`: switch back to the original session-backed base path before cleanup [`src/core/prompt-command-runtime.ts`]
        - `ensureReqResetMainBranch(...)`: switch the main worktree back to the main branch when HEAD rests on a generated branch [`src/core/req-reset-command.ts`]
          - `resolveReqResetMainBranchName(...)`: resolve the repository main branch from origin HEAD plus local conventions [`src/core/req-reset-command.ts`]
      ...
      - `notifyContextRetentionReminder(...)`: emit the final retained-context info notification with the current context-usage percentage after `req-reset` success [`src/index.ts`]
    - `registerReqReferencesCommand(...)`: register the specialized non-agentic `req-references` command that rejects non-`idle` invocation by transitioning workflow state to `error`, otherwise validates git state, regenerates `REFERENCES.md`, stages only that file, creates the fixed commit, verifies clean repository state, and returns to `idle` without worktree routing [`src/index.ts`]
      ...
      - `executeReqReferencesCommandExecution(...)`: regenerate `REFERENCES.md`, stage only the target file, create the fixed commit through the guarded commit helper when staged changes exist, and verify clean repository state [`src/core/req-references-command.ts`]
        - `runReferences(...)`: generate summarize-equivalent references markdown, overwrite `REFERENCES.md`, and return the status-only success payload [`src/core/tool-runner.ts`]
          - `runSummarize(...)`: execute the project summary markdown runner reused as the reference-file source [`src/core/tool-runner.ts`]
        - `getGitAddTargetPath(...)`: resolve the staged path relative to the git root [`src/core/req-references-command.ts`]
        - `runGuardedGitCommit(...)`: skip commit creation or run `git commit -m` based on a staged-changes precheck [`src/core/req-references-command.ts`]
        ...
        - `listResidualGitStatusLines(...)`: confirm no residual git-status rows remain after commit [`src/core/req-references-command.ts`]
          - `buildIgnoredGitStatusPaths(...)`: ignore extension-owned debug-log artifacts during cleanliness verification [`src/core/req-references-command.ts`]
        - `notifyContextRetentionReminder(...)`: emit the final retained-context info notification with the current context-usage percentage after direct references success [`src/index.ts`]
    ...
    - `registerExtensionStatusHooks(...)`: register shared wrappers for lifecycle telemetry hooks and suppress the documented stale-extension-context callbacks that can arrive after successful session replacement [`src/index.ts`]
      - `handleExtensionStatusEvent(...)`: process intercepted hook payloads through shared status helpers, tool-result debug logging, dedicated workflow-closure plus shutdown diagnostics built from pre-shutdown prompt snapshots, workflow-state transitions, recover a live bootstrap cwd during `session_start` when stale context mirrors still point at deleted worktrees, classify the prompt outcome on `agent_end` and defer matched-success prompt-command restoration plus stash-assisted merge and worktree deletion to `agent_settled` when the host supports that 0.80.4+ event because its `switchSession` call awaits the active agent run to become idle and would deadlock inside `agent_end`, or execute the same finalization directly at `agent_end` on legacy hosts that never emit `agent_settled`, warning-only closure notifications when restored `base-path` changes are reapplied, stale-safe closure notifications, and matched-failure worktree-execution-session retention plus closure-error notification and workflow `error` parking without resuming `base-path` so the failure stays visible, leaving `base-path` restoration plus worktree and branch cleanup to `req-reset`, and emitting the final context-retention reminder info notification after successful worktree-backed or no-worktree prompt closure [`src/index.ts`]
        - `isPiAgentSettledEventSupported(...)`: probe the running pi host for `agent_settled` event availability from whether lifecycle event registration returns an unsubscribe function and cache the result on the status controller state [`src/index.ts`]
        - `finalizeMatchedPromptSuccess(...)`: finalize a matched successful worktree-backed prompt at the current lifecycle point by preserving the execution transcript, restoring the original session-backed base path through the persisted replacement-session context, executing the stash-assisted `base-path` merge, deleting the worktree plus branch, surfacing error or warning-only notifications, clearing pending and active prompt state, transitioning workflow state through `merging` to `idle`, and emitting the final context-retention reminder info notification on successful closure [`src/index.ts`]
          - `notifyContextRetentionReminder(...)`: emit the final retained-context info notification with the current context-usage percentage after successful worktree-backed closure [`src/index.ts`]
