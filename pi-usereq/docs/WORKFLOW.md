      ...
      - `registerExtensionOffline(...)`: resolve paths, load the extension default export, and invoke registration under the requested cwd [`scripts/lib/extension-debug-harness.ts`]
        ...
        - `withProcessCwd(...)`: execute registration under the requested cwd [`scripts/lib/extension-debug-harness.ts`]
          - `piUsereqExtension(...)`: register extension commands, tools, shortcuts, and lifecycle hooks [`src/index.ts`]
            ...
            - `registerPiNotifyShortcut(...)`: register the configured sound-level shortcut when the runtime supports shortcuts [`src/index.ts`]
      ...
      - `withProcessCwd(...)`: execute `session_start` under the requested cwd [`scripts/lib/extension-debug-harness.ts`]
        ...
        - `handleExtensionStatusEvent(...)`: process the replayed `session_start` payload through shared status telemetry helpers [`src/index.ts`]
          ...
          - `updateExtensionStatus(...)`: refresh stored context usage, elapsed timers, cumulative runtime, and rendered status text [`src/core/extension-status.ts`]
            ...
            - `renderPiUsereqStatus(...)`: render the base/context/elapsed/notify/beep/sound footer with CLI-theme token overlays [`src/core/extension-status.ts`]
              ...
              - `buildPiUsereqStatusText(...)`: compose the single-line footer payload with consolidated elapsed semantics [`src/core/extension-status.ts`]
                ...
                - `formatPiNotifyStatus(...)`: serialize the global command-notify flag for the footer [`src/core/pi-notify.ts`]
                - `formatPiNotifyBeepStatus(...)`: serialize the global beep flag for the footer [`src/core/pi-notify.ts`]
          - `runPiNotifyEffects(...)`: dispatch configured command-notify, terminal-beep, sound, and optional Pushover hooks on `agent_end` [`src/core/pi-notify.ts`]
            ...
            - `shouldRunPiNotifyBeep(...)`: validate global terminal-beep enablement plus the matching event toggle [`src/core/pi-notify.ts`]
              - `isPiNotifyOutcomeEnabled(...)`: map `end|esc|err` to the selected toggle set [`src/core/pi-notify.ts`]
            - `runPiNotifyBeep(...)`: emit the terminal bell control byte [`src/core/pi-notify.ts`]
            ...
            - `runPiNotifyCommand(...)`: execute `PI_NOTIFY_CMD` with runtime placeholder substitution [`src/core/pi-notify.ts`]
              - `substitutePiNotifyShellTemplate(...)`: replace installation, prompt, base, time, and args placeholders inside the shell template [`src/core/pi-notify.ts`]
    ...
    - `replayCommand(...)`: replay a recorded command handler and capture prompt/UI side effects [`scripts/lib/extension-debug-harness.ts`]
      ...
      - `withProcessCwd(...)`: execute the named command under the requested cwd [`scripts/lib/extension-debug-harness.ts`]
        ...
        - `configurePiUsereq(...)`: execute the interactive configuration menu [`src/index.ts`]
          ...
          - `renderPiUsereqStatus(...)`: render the base/context/elapsed/notify/beep/sound footer after save-and-close with CLI-theme token overlays [`src/core/extension-status.ts`]
            - `createStatusThemeAdapter(...)`: constrain status rendering to CLI-supported `accent`, `warning`, `dim`, and `error` theme tokens [`src/core/extension-status.ts`]
            - `buildPiUsereqStatusText(...)`: compose the single-line footer payload with consolidated elapsed semantics plus notify and sound status fields [`src/core/extension-status.ts`]
              - `formatStatusField(...)`: render one colored text field [`src/core/extension-status.ts`]
              - `formatRenderedStatusField(...)`: render one pre-styled field [`src/core/extension-status.ts`]
              - `formatContextUsageBar(...)`: render the 10-cell context bar or threshold-specific overlay [`src/core/extension-status.ts`]
              - `formatElapsedStatusValue(...)`: compose `⏱︎`, `⚑`, and `⌛︎` timer segments [`src/core/extension-status.ts`]
          ...
          - `configurePiNotifyMenu(...)`: interactive notify, beep, sound, and Pushover editor [`src/index.ts`]
            - `buildPiNotifyMenuChoices(...)`: serialize command-notify, terminal-beep, sound, and nested Pushover rows for the shared settings menu [`src/index.ts`]
...
- Internal Call-Trace Tree:
    ...
    - `registerExtensionStatusHooks(...)`: register shared wrappers for lifecycle telemetry hooks [`src/index.ts`]
      - `handleExtensionStatusEvent(...)`: process intercepted hook payloads through shared status helpers [`src/index.ts`]
        ...
        - `updateExtensionStatus(...)`: refresh stored context usage, elapsed timers, prompt-request state, cumulative runtime, and rendered status text [`src/core/extension-status.ts`]
          - `refreshContextUsage(...)`: sync the latest context-usage snapshot [`src/core/extension-status.ts`]
          - `syncPiUsereqStatusTicker(...)`: start or stop the live elapsed ticker [`src/core/extension-status.ts`]
          - `renderPiUsereqStatus(...)`: render the base/context/elapsed/notify/beep/sound/pushover footer with CLI-theme token overlays [`src/core/extension-status.ts`]
            - `createStatusThemeAdapter(...)`: constrain status rendering to CLI-supported `accent`, `warning`, `dim`, and `error` theme tokens [`src/core/extension-status.ts`]
            - `buildPiUsereqStatusText(...)`: compose the single-line footer payload with consolidated elapsed semantics plus notify, sound, and Pushover status fields [`src/core/extension-status.ts`]
              - `formatStatusField(...)`: render one colored text field [`src/core/extension-status.ts`]
              - `formatRenderedStatusField(...)`: render one pre-styled field [`src/core/extension-status.ts`]
              - `formatContextUsageBar(...)`: render the 10-cell context bar or threshold-specific overlay [`src/core/extension-status.ts`]
                - `resolveContextUsageOverlay(...)`: choose `◀ CLEAR ▶ ` or ` ◀ FULL ▶ ` overlay states with CLI-supported theme tokens [`src/core/extension-status.ts`]
        ...
        - `runPiNotifyEffects(...)`: dispatch configured command-notify, terminal-beep, sound, and optional Pushover API requests on `agent_end` [`src/core/pi-notify.ts`]
          - `classifyPiNotifyOutcome(...)`: map assistant stop reasons to `end`, `esc`, or `err` [`src/core/pi-notify.ts`]
            - `hasAgentEndStopReason(...)`: scan assistant stop reasons inside the `agent_end` payload [`src/core/pi-notify.ts`]
          - `shouldRunPiNotifyBeep(...)`: validate global terminal-beep enablement plus the matching event toggle [`src/core/pi-notify.ts`]
            - `isPiNotifyOutcomeEnabled(...)`: map `end|esc|err` to the selected toggle set [`src/core/pi-notify.ts`]
  ...
  - `configurePiUsereq(...)`: interactive configuration menu runtime root [`src/index.ts`]
    ...
    - `renderPiUsereqStatus(...)`: render the base/context/elapsed/notify/beep/sound/pushover footer after save-and-close with CLI-theme token overlays [`src/core/extension-status.ts`]
      - `createStatusThemeAdapter(...)`: constrain status rendering to CLI-supported `accent`, `warning`, `dim`, and `error` theme tokens [`src/core/extension-status.ts`]
      - `buildPiUsereqStatusText(...)`: compose the single-line footer payload with consolidated elapsed semantics plus notify, sound, and Pushover status fields [`src/core/extension-status.ts`]
        - `formatStatusField(...)`: render one colored text field [`src/core/extension-status.ts`]
        - `formatRenderedStatusField(...)`: render one pre-styled field [`src/core/extension-status.ts`]
    ...
    - `configurePiNotifyMenu(...)`: interactive notify, beep, sound, and Pushover editor [`src/index.ts`]
      - `buildPiNotifyMenuChoices(...)`: serialize command-notify, terminal-beep, sound, and nested Pushover rows for the shared settings menu [`src/index.ts`]
      - `togglePiNotifyFlag(...)`: toggle one persisted boolean notification setting [`src/index.ts`]
      - `selectPiNotifySoundLevel(...)`: present the `Selected sound command` selector [`src/index.ts`]
      - `configurePiNotifyPushoverMenu(...)`: interactive Pushover editor nested under the notifications menu [`src/index.ts`]
        - `buildPiNotifyPushoverMenuChoices(...)`: serialize global enablement, per-event toggles, priority, title, text, and credential actions [`src/index.ts`]
        - `selectPiNotifyPushoverPriority(...)`: present the `Normal|High` priority selector [`src/index.ts`]
