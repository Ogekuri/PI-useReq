  ...
  - `main(...)`: parse argv, dispatch inspect/session-start/command/tool/sdk-smoke, and serialize the selected report [`scripts/debug-extension.ts`]
    ...
    - `replayCommand(...)`: replay a recorded command handler and capture prompt/UI side effects [`scripts/lib/extension-debug-harness.ts`]
      ...
      - `withProcessCwd(...)`: execute the named command under the requested cwd [`scripts/lib/extension-debug-harness.ts`]
        ...
        - `configurePiUsereq(...)`: execute the interactive configuration menu [`src/index.ts`]
          ...
          - `configurePiNotifyMenu(...)`: interactive notify, sound, and Pushover editor with dedicated event submenus [`src/index.ts`]
            - `buildPiNotifyMenuChoices(...)`: serialize command-notify, sound, and Pushover blocks plus shared-event submenu launchers [`src/index.ts`]
              ...
              - `buildPiNotifyPushoverRows(...)`: serialize direct Pushover rows plus the shared event-submenu launcher [`src/index.ts`]
            - `configurePiNotifyEventMenu(...)`: interactive completed/interrupted/failed event editor for one notification transport [`src/index.ts`]
              - `buildPiNotifyEventMenuChoices(...)`: serialize shared event rows plus submenu reset/save actions [`src/index.ts`]
    ...
    - `replayTool(...)`: replay a recorded tool execute handler and capture content/details [`scripts/lib/extension-debug-harness.ts`]
      ...
      - `withProcessCwd(...)`: execute the named tool under the requested cwd [`scripts/lib/extension-debug-harness.ts`]
        ...
        - `buildFailedTokenToolExecuteResult(...)`: normalize pre-payload token failures into the stable structured token-tool response shape [`src/index.ts`]
          - `normalizeToolFailure(...)`: convert `ReqError` failures into deterministic execution metadata [`src/core/agent-tool-json.ts`]
          - `buildStructuredToolExecuteResult(...)`: mirror the final JSON object into tool content and details [`src/core/agent-tool-json.ts`]
