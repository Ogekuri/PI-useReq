      ...
      - `withProcessCwd(...)`: execute the named command under the requested cwd [`scripts/lib/extension-debug-harness.ts`]
        ...
        - `configurePiUsereq(...)`: execute the interactive configuration menu [`src/index.ts`]
          - `buildPiUsereqMenuChoices(...)`: serialize top-level settings actions plus the display-only config-path row [`src/index.ts`]
            - `formatProjectConfigPathForMenu(...)`: format the current config path and rewrite a leading `$HOME` token to `~` for menu display [`src/index.ts`]
          ...
          - `renderPiUsereqStatus(...)`: render the base/context/elapsed/sound footer after save-and-close with CLI-theme token overlays [`src/core/extension-status.ts`]
            ...
            - `buildPiUsereqStatusText(...)`: compose the single-line footer payload with consolidated elapsed semantics and sound status fields [`src/core/extension-status.ts`]
              ...
              - `formatElapsedStatusValue(...)`: compose `⏱︎`, `⚑`, and `⌛︎` timer segments [`src/core/extension-status.ts`]
                - `formatCompletedStatusDuration(...)`: render completed timers or the unset placeholder [`src/core/extension-status.ts`]
                - `formatStatusDuration(...)`: render `M:SS` durations [`src/core/extension-status.ts`]
          - `configureStaticCheckMenu(...)`: interactive guided static-check editor [`src/index.ts`]
          - `configurePiUsereqToolsMenu(...)`: interactive active-tool editor without a status-reference action [`src/index.ts`]
...
- Internal Call-Trace Tree:
  ...
  - `configurePiUsereq(...)`: interactive configuration menu runtime root [`src/index.ts`]
    ...
    - `configureStaticCheckMenu(...)`: interactive guided static-check editor [`src/index.ts`]
      ...
      - `buildStaticCheckMenuChoices(...)`: serialize guided add/remove/reset static-check actions [`src/index.ts`]
